	...
	<script>
	   var bookedTimes = ${bookedTimes};
	    ...
	    $(document).ready(function() {
	        var dateInput = $('input[name="datevisit"]');
	        var timeSelect = $('#time');

	        dateInput.change(function() {
	            var date = $(this).val();

	            // Disable times that are already booked on this date
	            if (bookedTimes[date]) {
	                timeSelect.find('option').prop('disabled', false);

	                for (var i = 0; i < bookedTimes[date].length; i++) {
	                    timeSelect.find('option[value="' + bookedTimes[date][i] + '"]').prop('disabled', true);
	                }
	            }
	        });

	        // Disable dates that have reached the maximum bookings
	        dateInput.each(function() {
	            var date = $(this).val();

	            if (bookedTimes[date] && bookedTimes[date].length >= 6) {
